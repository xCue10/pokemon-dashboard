...
import { getCards, createCard, updateCard, deleteCard, refreshCardPrice, exportCards } from '../utils/api';
...
export default function Collection() {
  ...
  const handleRefreshPrice = async (id) => {
    setRefreshingId(id);
    try {
      const updated = await refreshCardPrice(id);
      toast.success(`Price updated: ${formatCurrency(updated.market_price)}`);
                ...
                <tr key={card.id} className="table-row-hover">
                  ...
                  <td className="table-td">
                    <div className="flex items-center gap-1">
                      ...
                      {card.pokemon_tcg_id && (
                        <button
                          onClick={() => handleRefreshPrice(card.id)}
                          ...
                          title="Refresh price"
                        >
                          {refreshingId === card.id ? <span className="spinner" style={{ width: 14, height: 14, borderWidth: 2 }} /> : '🔄'}
                        </button>
                      )}
                      <button
